...
![A sentence rendered in an Apple text view with a blue cat glyph flowing inline](docs/images/inline-glyph.png)

The images in this README are real `NSAdaptiveImageGlyph` output rendered
through TextKit 2, generated by
[`Scripts/generate-readme-images.swift`](Scripts/generate-readme-images.swift).
...
Add the package with a library-compatible version range:
...
AdaptiveGlyphKit has no third-party dependencies.
...
Because the glyph is a real adaptive image glyph, it tracks the surrounding
font — body text, titles, and large Dynamic Type sizes all render it at the
right scale:

![The same glyph rendered inline at body, title, display, and accessibility text sizes](docs/images/dynamic-type.png)
